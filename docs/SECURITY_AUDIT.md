...
- **`register_vk` / `disable_vk` — 🔴 CRITICAL access-control bypass.** Both entry points call `require_owner_or_gov(governance_approved)`, where `governance_approved: u64` is a **plain caller-supplied named argument on the deploy**, not a value read from the `governance` contract on-chain:
  ...
  - **Fix status (2026-07-27):** the bypass is fixed in source (`require_owner_or_gov` now delegates to `require_owner()` unconditionally) and covered by a new regression test (`gov_approved_flag_no_longer_bypasses_owner_check`) — 105/105 contract tests pass, and the rebuilt wasm is MVP-clean at 52,353 bytes (under the 65,536 cap). **The fix is not yet live on testnet.** Three redeploy attempts (from the sandbox building this fix, using the correct `anna-stolbovskaja` deployer key) all reverted on-chain with `ApiError::InvalidArgument`, at an identical gas cost regardless of wasm content — including a control test that rebuilt the *unmodified* original source through the same pipeline and hit the exact same revert. That rules out the code fix itself as the cause; it points at a build-toolchain or network/protocol compatibility issue with installing new contract versions from this environment right now (the original deploy 2 days ago went through an authenticated `cspr.cloud` RPC endpoint; these attempts used the unauthenticated public node since no `cspr.cloud` API key was available). The live contract is unaffected by the failed attempts — still the original vulnerable version.
  - **Until redeployed, treat `zk-verifier.register_vk`/`disable_vk` as unauthenticated in production/judging conversations** — the fix exists and is tested but is not yet the code actually running on testnet.
  - **Verdict:** 🔴 **Not safe as deployed.** Fix written and tested; deployment blocked on a tooling/network issue under investigation. Top-priority item to close out from this audit.
