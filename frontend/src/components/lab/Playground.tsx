...
const playgroundEndpoints: EndpointConfig[] = [
  ...
  { name: 'POST /proofs', method: 'POST', path: '/proofs', description: 'Create a new ZK proof.', exampleBody: JSON.stringify({ agent: 'agent-alpha', input: 'loan_application_42', output: 'approved_with_conditions', model: 'gpt-4o', use_case: 'kyc-eligibility' }, null, 2), apiCall: api.createProof },
  { name: 'POST /verify', method: 'POST', path: '/verify', description: 'Verify an existing ZK proof.', exampleBody: JSON.stringify({ proof_id: 'P-5' }, null, 2), apiCall: api.verifyProof },
  { name: 'POST /proofs/{id}/revoke', method: 'POST', path: '/proofs/{id}/revoke', description: 'Revoke a specific ZK proof.', apiCall: (params) => api.revokeProof(params.id), params: [{ name: 'id', type: 'string', optional: false, default: 'P-5' }] },
  ...
  { name: 'POST /inference/prove', method: 'POST', path: '/inference/prove', description: 'Run inference and generate a proof.', exampleBody: JSON.stringify({ model_id: 'fraud-detection-v1', input: '{"amount": 1000, "risk": "high"}', agent: 'agent-alpha', output: '{"decision": "approved"}', use_case: 'kyc-eligibility' }, null, 2), apiCall: api.inferenceProve },
  { name: 'POST /inference/verify', method: 'POST', path: '/inference/verify', description: 'Verify an AI inference proof.', exampleBody: JSON.stringify({ model_id: 'fraud-detection-v1', proof_id: 'P-5', input: '{"amount": 1000, "risk": "high"}' }, null, 2), apiCall: api.inferenceVerify },
  ...
  { name: 'POST /zk/verify-groth16', method: 'POST', path: '/zk/verify-groth16', description: 'Verify a Groth16 ZK proof.', exampleBody: JSON.stringify({ proof: '{"pi_a": ["0x...", "0x..."], "pi_b": [["0x...", "0x..."], ["0x...", "0x..."]], "pi_c": ["0x...", "0x..."]}', public_signals: ['0xsignal1', '0xsignal2'] }, null, 2), apiCall: api.verifyGroth16 },
  ...
  { name: 'POST /pq/sign-sphincs', method: 'POST', path: '/pq/sign-sphincs', description: 'Sign using SPHINCS+ post-quantum cryptography.', exampleBody: JSON.stringify({ message: 'This is a test message for SPHINCS+ signing.' }, null, 2), apiCall: api.signSphincs },
  { name: 'POST /pq/verify-sphincs', method: 'POST', path: '/pq/verify-sphincs', description: 'Verify a SPHINCS+ signature.', exampleBody: JSON.stringify({ message: 'This is a test message for SPHINCS+ signing.', signature: '0x...', public_key: '0x...' }, null, 2), apiCall: api.verifySphincs },
  ...
  return (
    <div className="p-4">
      <SectionIntro
        ...
        dataSource="All requests hit the live CasperProver API (casperprover-api.onrender.com). Every result is real."
        badge="Live API"
        badgeColor="blue"
      />
