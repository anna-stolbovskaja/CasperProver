...
func TestPQKeyRing_HTTPRoundTrip_CreateSignVerify(t *testing.T) {
	...
	// Create ed25519 key.
	resp := doJSON(t, mux, "POST", "/v1/pq/keys", map[string]string{"algo": "ed25519"})
	...
	resp = doJSON(t, mux, "POST", "/v1/pq/keys/sign", map[string]string{
		"algo":    "ed25519",
		"message": "hello",
	})
	...
	// Verify — pass.
	resp = doJSON(t, mux, "POST", "/v1/pq/keys/verify", map[string]string{
		"key_id":    signed.KeyID,
		"message":   "hello",
...
func TestPQKeyRing_HTTPRotation(t *testing.T) {
	...
	// v1
	resp := doJSON(t, mux, "POST", "/v1/pq/keys", map[string]string{"algo": "mldsa65"})
	if resp.Code != http.StatusCreated {
		t.Fatalf("create v1: %d body=%s", resp.Code, resp.Body.String())
	}
	// rotate → v2
	resp = doJSON(t, mux, "POST", "/v1/pq/keys/mldsa65/rotate", nil)
	if resp.Code != http.StatusOK {
	...
	resp = doJSON(t, mux, "GET", "/v1/pq/keys?algo=mldsa65", nil)
	if resp.Code != http.StatusOK {
...
func TestPQKeyRing_HTTPMigrate(t *testing.T) {
	...
	_ = doJSON(t, mux, "POST", "/v1/pq/keys", map[string]string{"algo": "ed25519"})
	resp := doJSON(t, mux, "POST", "/v1/pq/keys/sign", map[string]string{
		"algo":    "ed25519",
	...
	// Create a hybrid key.
	_ = doJSON(t, mux, "POST", "/v1/pq/keys", map[string]string{"algo": "hybrid_ed25519_mldsa65"})

	// Migrate.
	resp = doJSON(t, mux, "POST", "/v1/pq/keys/migrate", map[string]string{
...
func TestPQKeyRing_DisabledGate(t *testing.T) {
	...
	resp = doJSON(t, mux, "GET", "/v1/pq/keys", nil)
	if resp.Code != http.StatusOK {
...
func TestPQKeyRing_UnknownAlgo(t *testing.T) {
	_, mux := buildKeyRingServer(t, true)
	resp := doJSON(t, mux, "POST", "/v1/pq/keys", map[string]string{"algo": "kyber-space-fluff"})
