...
// writeJSON is a small helper to keep the tenant handlers self-contained
// without touching the (larger) shared response helpers in server.go.
// It writes {"error": msg} on error via s.jsonError elsewhere; this
// helper is only for success bodies.
func writeJSON(w http.ResponseWriter, status int, body any) {
	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	w.WriteHeader(status)
	if err := json.NewEncoder(w).Encode(body); err != nil {
		// If encoding fails after WriteHeader we can't send a proper
		// error; the connection is already committed. Log via slog
		// through a real Server pointer would be nicer, but the
		// helper is intentionally stateless.
		_, _ = fmt.Fprintln(w, `{"error":"encode failed"}`)
	}
}
