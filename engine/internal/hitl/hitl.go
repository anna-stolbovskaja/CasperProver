...
const (
	...
	// ActionVeto — the policy hard-rejects the decision.
...
// return.
func (s *Service) Decide(commit decision.DecisionCommit) (Response, error) {
	if s == nil {
		return Response{}, errors.New("hitl: nil service")
	}
	// 1. Critical REJECT ⇒ VETO (mirrors the aggregate).
	if s.policy.VetoOnCriticalReject && commit.Aggregate == decision.VerdictReject && commit.VetoedBy != "" {
		return Response{
			Action: ActionVeto,
	...
	if s.policy.EscalateOnCriticalAbstain {
		for _, v := range commit.FacetVerdicts {
			...
			if v.Verdict == decision.VerdictAbstain {
...
func (s *Service) openTicket(commit decision.DecisionCommit, reason string) (Ticket, error) {
	t := Ticket{
		ID:         s.idgen(),
...
// isCriticalKind mirrors decision.FacetKind.isCritical without leaking
// unexported code across the package boundary.
func isCriticalKind(k decision.FacetKind) bool {
	return k == decision.FacetSafety || k == decision.FacetEquivocation
}
...
func nonCriticalApproveConfidence(verdicts []decision.FacetVerdict) (float64, int) {
	var sum float64
	var n int
	for _, v := range verdicts {
		...
		if v.Verdict != decision.VerdictApprove {
			continue
		}
		sum += v.Confidence
		n++
