...
- Backend retry logic on the Casper deploy submitter — exponential backoff, max 3 attempts, terminal-vs-transient error classification, mock-friendly `txPutter` interface for unit tests (10 new tests).
- Public/admin API auth split: explicit-public routes (`POST /proofs`, `POST /verify`, all GET/HEAD/OPTIONS) require no key; admin-gated routes (kyc, aggregation-finalize, model-registration, admin/*) require `X-API-Key`. Middleware returns 401 for missing header, 403 for wrong header, JSON error body on both. Constant-time key comparison retained.
- Per-wallet API-key issuance: `POST /admin/keys/issue` mints `sk_live_<64 hex>`, returned exactly once; only `sha256(key)` persisted in a new `api_keys` table (id, key_hash UNIQUE, wallet_addr, scope, created_at, revoked, revoked_at). `POST /admin/keys/revoke` flips the row and returns 404 on double-revoke. Admin-gated via the T2 middleware.
